...
// Define the CareerTemplate interface (matching the one from careerMatching.ts)
 ...
 */
function careerTypeToTitle(careerType: string): string {
  return careerType
    .split('-')
 ...
 */
function generatePlaceholderTemplate(careerType: string): CareerTemplate {
...
async function main() {
  try {
    ...
    const missingCareerTypes: string[] = JSON.parse(missingTemplatesData);
    ...
    const placeholderTemplates: CareerTemplate[] = [];
